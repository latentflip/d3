...
(function() {
  ...
  require(['d3', 'underscore'], function(d3, _) {
    var cluster, diagonal, height, json, render, vis, width;
    json = {
      id: 1,
      name: "today",
      children: [
    ...
    vis = d3.select("body").append("svg").attr("width", width).attr("height", height).append("g").attr("transform", "translate(40, 0)");
    render = function(json) {
      var link, node, nodes;
      nodes = cluster.nodes(json);
      link = vis.selectAll("path.link").data(cluster.links(nodes)).enter().append("path").attr("class", "link").attr("d", diagonal);
